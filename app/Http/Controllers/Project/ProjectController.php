...
class ProjectController extends Controller
{
    ...
    public function show(Request $request, Project $project)
    {
        ...
        if ($user) {
            ...
            $project = Project::with(['user:id,tagname,first_name,last_name,photo_url,email,expertise', 'skills', 'requirements', 'project_team.members.member', 'project_review', 'is_wished' =>
            function ($q) use ($user) {
                $q->where('wishlists.user_id', $user->id);
        ...
        $project = Project::with(['user:id,tagname,first_name,last_name,photo_url,email,expertise', 'skills', 'requirements', 'project_team.members.member'])->findOrFail($project->id);
    ...
    public function similarProjects(Request $request, Project $project)
    {
        ...
        if ($user) {
            ...
            if ($expertise === 'UI/UX Designer') {
                $expertise = 'ui_ux_designer';
            } else if ($expertise === 'Frontend Engineer') {
                $expertise = 'front_end_engineer';
            } else if ($expertise === 'Backend Engineer') {
                $expertise = 'back_end_engineer';
            } else if ($expertise === 'Data Expert') {
                $expertise = 'data_expert';
            }
