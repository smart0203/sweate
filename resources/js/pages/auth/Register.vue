  ...
  <div class="register-page--container" :class="{ 'lecturer-bg': lecturerRole }">
    ...
    <div class="resgiter-form--container">
      <router-link v-if="$matchMedia.xl" :to="{ name: 'index' }" class="mx-auto mb-1">
        <img class="desktop-nav__logo" src="/images/logo.png" alt="">
      </router-link>
      ...
      <div class="login-role--container">
        <div>
          <input id="login-student" v-model="form.role" class="login-radio" type="radio" value="Start a Project" name="role">
          <label class="login-radio--label" for="login-student" @click="chooseStudent">Find a Project</label>
        </div>
        <div>
          <input id="login-lecturer" v-model="form.role" class="login-radio" type="radio" value="Find a Project" name="role">
          <label class="login-radio--label" for="login-lecturer" @click="chooseLecturer">Start a Project</label>
        </div>
      </div>
      ...
      <div v-if="$matchMedia.xl" class="desktop-login__footer">
        Sweatequity, All Rights Reserved. &copy; {{new Date().getFullYear()}} . | Developed by Sweatequity
      </div>
    </div>
  </div>
</template>
